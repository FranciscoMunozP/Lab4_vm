...
func (s *server) StoreToFile(ctx context.Context, req *pb.ChoiceSave) (*pb.Confirmation, error) {
	...
	if req.Floor == "1" {
		if req.Choice == "0" {
			_, err = file.WriteString("Mercenario " + req.MercenaryId + " escogió la Escopeta")
		} else if req.Choice == "1" {
			_, err = file.WriteString("Mercenario " + req.MercenaryId + " escogió el Rifle automático")
		} else if req.Choice == "2" {
			_, err = file.WriteString("Mercenario " + req.MercenaryId + " escogió los Puños eléctricos")
		}
	} else if req.Floor == "2" {
		pasillo := ""
		if req.Choice == "0" {
		...
		_, err = file.WriteString("Mercenario " + req.MercenaryId + " escogió el pasillo " + pasillo)
	} else if req.Floor == "3" {
